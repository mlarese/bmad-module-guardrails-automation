...
retention, proponi `gau-profile` chiedendo il linguaggio del dominio — è il segnale che lo instrada, non un'azione da passare — prima di fissare lo schema. Se l'utente deve
...
| Linguaggio del dominio e decisione | Termini condivisi, casi limite, provenienza e condizioni che riaprono il modello | `gau-profile`, chiedendo il glossario di dominio; l'esito sta in `{project-root}/_bmad/memory/grl-shared/domain-glossary.md` |
