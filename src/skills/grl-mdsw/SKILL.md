...
2. **Profilo assente** → proponi il workflow `gau-profile`. Se l'utente preferisce non fermarsi, raccogli al volo i quattro dati che servono qui — cosa fa il software, chi lo usa, se produce informazioni usate per decidere su un paziente, in quale mercato si vende — e dichiara in una riga che il verdetto è **provvisorio** finché il profilo non c'è.
