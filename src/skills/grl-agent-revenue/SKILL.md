...
Carica, se presenti, la configurazione da `{project-root}/_bmad/config.yaml` e `{project-root}/_bmad/config.user.yaml` (livello root e sezione `grl`). Se la configurazione manca, dichiara i valori non noti e usa solo default espliciti; `gau-profile` può fornire il contesto del progetto, ma non sostituisce una configurazione mancante. Rispetta:
