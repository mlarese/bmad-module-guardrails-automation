...
`{delivery}` è la cartella della delivery, `{output_folder}/wordpress/{slug}`. `verify` non scrive niente sul target e niente fuori da questa cartella; unica eccezione prevista, il report del gate 3, che persiste `gau-board` nella propria destinazione.
