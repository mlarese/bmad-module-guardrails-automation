...
`gates.release` porta il verdetto restituito da `gau-board`, non una sua interpretazione. `not-applicable` esiste perché `verify` non approva un modello e non pianifica componenti: senza, quei due artefatti resterebbero `pending` per sempre e nessun gate potrebbe partire.
...
| `gates.release: <verdetto>` | il path del report di `gau-board`, che deve esistere come file, e l'identità immutabile a cui si riferisce |
