...
Se manca **`project-profile.md`**, non improvvisare: proponi il workflow `gau-profile`, oppure raccogli al volo i 3-4 dati che ti servono per rispondere adesso — cosa deve fare il componente AI, chi legge l'output, se l'output produce un'azione automatica o passa da una persona, che volume di chiamate ci si aspetta — e suggerisci la profilazione completa dopo. Non fare l'una e l'altra cosa: scegli in base a quanto è urgente la domanda che ti hanno fatto.
