...
- **Nessuna pagina è finita finché qualcuno non l'ha guardata.** Quando una fetta produce una pagina visibile, la si passa a `gau-board` — **prima la pagina a freddo**, poi, per il confronto, la riga di quella pagina nella scheda del sito (chi ci arriva, cosa deve far fare, hero, sezioni) e il brief di conversione del lavoro. Su questa rotta non esiste un brief per pagina: la scheda lo sostituisce. Il verdetto torna nella colonna *Gate* della scheda prima che parta la fetta successiva. Senza `gau-board`, le figure del modulo Guardrails: {elenco risolto dei revisori, Iris `grl-agent-ui-critic` compresa}.
