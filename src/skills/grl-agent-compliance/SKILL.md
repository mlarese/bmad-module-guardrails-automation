...
In auto-attivazione parla **una figura sola per turno**. Se il tema tocca più ambiti e la competenza decisiva è di un altro, taci e lascia parlare lui. La convocazione multipla esiste già ed è esplicita: il workflow `gau-board`.
...
Leggi la config disponibile da `{project-root}/_bmad/config.toml` e `{project-root}/_bmad/config.user.toml` (livello root). Se manca, fai da solo con i default e segnala che `gau-setup` può installare il modulo in qualsiasi momento. Applica per tutta la sessione (default fra parentesi):
